...
def paste_text(text: str, clipboard_fallback: bool = True, target_app=None) -> str:
    ...
    Flow:
      ...
      3. Try direct AX insertion (works for native + many web fields)
      4. If AX fails, Cmd+V as last resort (target app is now focused)
    ...
    Returns:
        "pasted"    — text was inserted (AX or Cmd+V)
    ...
    # Step 0: Always put text on clipboard first — never lose the transcription
    _copy_to_clipboard(text)

    # Step 1: Bring the target app back to foreground
    app_name = target_app.localizedName() if target_app else "none"
    _dbg(f"activating target: {app_name}")
    _activate_app(target_app)

    # Step 2: Simulate Cmd+V — the only method that works reliably across
    # ALL apps (native, Electron, Catalyst, web content, Terminal).
    # AX insertion (AXSelectedText) reports success but silently fails in
    # Slack, WhatsApp, Terminal, and other non-standard text inputs.
